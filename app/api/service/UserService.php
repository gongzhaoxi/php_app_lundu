...
class UserService extends Service
{
    ...
    {
        $modelUser = new AuthAdmin();
        $user = $modelUser
            ->field(['id,role_id,nickname,username,avatar,phone,email,last_login_ip,last_login_time'])
            ->where(['id'=>$id])
            ->where(['is_delete'=>0])
            ->findOrEmpty()
            ->toArray();
    ...
    public static function info(int $id): array
    {
		...
		if(!empty($user['id'])){
			$menus = AuthPerm::field(true)->where(['role_id'=>intval($user['role_id'])])->column('menu_id');
			$user['no_auth'] = [];
			//4111航班信息 4112编辑航班
			if(!in_array(4111,$menus)){
				$user['no_auth'][] = 0;
			}
			if(!in_array(4112,$menus)){
				$user['no_auth'][] = 1;
			}		
		}
