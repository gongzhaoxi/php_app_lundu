...
class LoginService extends Service
{
    ...
    {
        // 登录限制
        $loginFailCount = Cache::get('login:fail:'.$account);
        ...
        // 账户验证
        if (!$adminUser) {
            Cache::set('login:fail:'.$account, intval($loginFailCount)+1, 600);
            throw new OperateException('用户名或密码错误,您还可以尝试['.$surplusCount.']次!');
        }

        // 账户密码
        $password = make_md5_str($password, $adminUser['salt']);
        ...
        // 登录账户
        $token = UserWidget::granToken(intval($adminUser['id']), $terminal);
        return ['token'=>$token] ?? [];
    }
