...
class ExcelDriver
{
    ...
    public static function export(array $fields, array $lists, array $options = []): string
    {
        ...
        foreach ($fields as $key => $vo) {
			$titleWidth[] = empty($vo['width'])?30:$vo['width'];
            $worksheet->setCellValue([$key+1, 1], $vo['name']);
        }
        ...
        $worksheet->getStyle([1, 1, count($fields), 1])->applyFromArray($option['headerStyle']);
        ...
        if ($option['exportMethod'] === 'path') {
            ...
            $writer->save($option['exportPath'] . $option['exportName'] . '.xlsx');
            return $option['exportPath'] . $option['exportName'] . '.xlsx';
        } else {
     ...
     * @return array
     */
    public static function import(string $path, array $fields, array $options = [],bool $clean_space = true): array
    {
        $option = [
            // 跳过第几行
        ...
        foreach ($sheetData as $rows) {
            ...
            foreach ($fields as $key => $field) {
                $data[$field] = remove_emoji($rows[$key],$clean_space);
            }
