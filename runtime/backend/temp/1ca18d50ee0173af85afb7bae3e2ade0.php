<?php /*a:2:{s:68:"/www/wwwroot/lundu.ecloudm.com/app/backend/view/auth/post/index.html";i:1755524162;s:66:"/www/wwwroot/lundu.ecloudm.com/app/backend/view/common/layout.html";i:1764860923;}*/ ?>
<!DOCTYPE html>
...
<script>
    layui.use(function() {
        ...
        let table = waitUtil.table({
            ...
            ,url: '<?php echo route("auth.post/index"); ?>'
            ,cols: [[
        ...
        waitUtil.event({
            add: function () {
                waitUtil.popup({
                    title: '新增',
                    url: '<?php echo route("auth.post/add"); ?>',
                    area: ['510px', '510px'],
            ...
            edit: function (obj) {
                waitUtil.popup({
                    title: '编辑',
                    url: '<?php echo route("auth.post/edit"); ?>?id='+obj.data.id,
                    area: ['510px', '510px'],
                    success: function (layero, index) {
                        layero.layui.form.on('submit(addForm)', function(data) {
                            waitUtil.locking(this);
                            ...
                            waitUtil.ajax({
                                url: '<?php echo route("auth.post/edit"); ?>',
                                type: 'POST',
                                data: data.field
                            }).then((res) => {
                                waitUtil.unlock(this);
            ...
            del: function (obj) {
                layer.confirm('确定要删除此项数据吗？', function(index) {
                    layer.close(index);
                    waitUtil.ajax({
                        url: '<?php echo route("auth.post/del"); ?>',
                        type: 'POST',
                        data: {id: obj.data.id}
                    }).then((res) => {
                        if (res.code === 0) {
