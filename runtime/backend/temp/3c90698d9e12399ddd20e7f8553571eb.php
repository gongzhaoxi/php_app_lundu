<?php /*a:2:{s:66:"/www/wwwroot/lundu.ecloudm.com/app/backend/view/sailing/index.html";i:1766846788;s:66:"/www/wwwroot/lundu.ecloudm.com/app/backend/view/common/layout.html";i:1764860923;}*/ ?>
<!DOCTYPE html>
        ...
        <div class="layui-form-item">
            <div class="layui-inline">
                ...
                <div class="layui-input-inline">
                    <input type="text" id="sailing_date" name="sailing_date"  value="<?php echo htmlentities((string) $date); ?>"  class="layui-input" placeholder="请输入日期" autocomplete="off">
                </div>
            </div>
            <div class="layui-inline">
...
<script>
    layui.use(['laydate'],function() {
        ...
        let table = waitUtil.table({
            ...
            ,url: '<?php echo route("Sailing/index"); ?>?sailing_date=<?php echo htmlentities((string) $date); ?>'
			,toolbar:false
            ,defaultToolbar: false
			,height:'full-210'
            ,cols: [[
                {type:"numbers",title:'序号', width:60, align:'center'},
				{field:'push_time', title:'提交时间', align:'center'},
				...
				{field:'push_by', title:'提交人', align:'center'},
				{field:'confirm_by', title:'确认人', align:'center'},
				{field:'confirm_time', title:'确认时间', align:'center'},
            ]]
        });
		
		$('.export').click(function(){
			exportData(layui.form.val('layui-search'),'<?php echo route("Sailing/export"); ?>',10000);
		})
        ...
        waitUtil.event({
            add: function () {
                waitUtil.popup({
                    title: '新增',
                    url: '<?php echo route("Sailing/add"); ?>',
                    area: ['90%', '90%'],
            ...
            edit: function (obj) {
                waitUtil.popup({
                    title: '编辑',
                    url: '<?php echo route("Sailing/edit"); ?>?id='+obj.data.id,
                    area: ['90%', '90%'],
                    success: function (layero, index) {
                        layero.layui.form.on('submit(addForm)', function(data) {
                            waitUtil.locking(this);
                            ...
                            waitUtil.ajax({
                                url: '<?php echo route("Sailing/edit"); ?>',
                                type: 'POST',
                                data: data.field
                            }).then((res) => {
                                waitUtil.unlock(this);
            ...
            del: function (obj) {
                layer.confirm('确定要删除此项数据吗？', function(index) {
                    layer.close(index);
                    waitUtil.ajax({
                        url: '<?php echo route("Sailing/del"); ?>',
                        type: 'POST',
                        data: {id: obj.data.id}
                    }).then((res) => {
                        if (res.code === 0) {
