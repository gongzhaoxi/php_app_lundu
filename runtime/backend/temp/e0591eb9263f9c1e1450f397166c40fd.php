<?php /*a:2:{s:73:"/www/wwwroot/lundu.ecloudm.com/app/backend/view/system/crontab/index.html";i:1755524162;s:66:"/www/wwwroot/lundu.ecloudm.com/app/backend/view/common/layout.html";i:1764860923;}*/ ?>
<!DOCTYPE html>
    ...
    <div class="layui-card">
        ...
        <div class="layui-card-body">
            ...
            <script type="text/html" id="table-operate">
                {{#  if(d.status === 1){ }}
                    <button type="button" class="layui-btn layui-btn-xs layui-btn-primary <?php echo check_perms('stop'); ?>" lay-event="stop" lay-tips="运行中">
                        <i class="layui-icon icon-stop"></i>
                    </button>
                {{#  } }}
                {{#  if(d.status === 2 || d.status === 3){ }}
                    <button type="button" class="layui-btn layui-btn-xs layui-btn-warm <?php echo check_perms('run'); ?>" lay-event="run" lay-tips="已暂停">
                        <i class="layui-icon icon-run"></i>
                    </button>
                {{#  } }}
                <button type="button" class="layui-btn layui-btn-xs layui-btn-default <?php echo check_perms('edit'); ?>" lay-event="edit" lay-tips="编辑">
                    <i class="layui-icon icon-edit"></i>
                </button>
                <button type="button" class="layui-btn layui-btn-xs layui-btn-danger <?php echo check_perms('del'); ?>" lay-event="del" lay-tips="删除">
...
<script>
    layui.use(function() {
        ...
        let table = waitUtil.table({
            ...
            ,url: '<?php echo route("system.crontab/index"); ?>'
            ,cols: [[
        ...
        waitUtil.event({
            add: function () {
                waitUtil.popup({
                    title: '新增',
                    url: '<?php echo route("system.crontab/add"); ?>',
                    area: ['500px', '500px'],
            ...
            edit: function (obj) {
                waitUtil.popup({
                    title: '编辑',
                    url: '<?php echo route("system.Crontab/edit"); ?>?id='+obj.data.id,
                    area: ['500px', '500px'],
                    success: function (layero, index) {
                        layero.layui.form.on('submit(addForm)', function(data){
                            waitUtil.locking(this);
                            ...
                            waitUtil.ajax({
                                url: '<?php echo route("system.Crontab/edit"); ?>',
                                type: 'POST',
                                data: data.field
                            }).then((res) => {
                                waitUtil.unlock(this);
            ...
            del: function (obj) {
                layer.confirm('确定删除该任务吗?', function(index) {
                    layer.close(index);
                    waitUtil.ajax({
                        url: '<?php echo route("system.crontab/del"); ?>',
                        type: 'POST',
                        data: {id: obj.data.id}
                    }).then((res) => {
                        if (res.code === 0) {
            ...
            stop: function (obj) {
                layer.confirm('确定要停止运行吗?', function(index) {
                    layer.close(index);
                    waitUtil.ajax({
                        url: '<?php echo route("system.crontab/stop"); ?>',
                        type: 'POST',
                        data: {id: obj.data.id}
                    }).then((res) => {
                        if (res.code === 0) {
            ...
            run: function (obj) {
                layer.confirm('确定要启动任务吗?', function(index) {
                    layer.close(index);
                    waitUtil.ajax({
                        url: '<?php echo route("system.Crontab/run"); ?>',
                        type: 'POST',
                        data: {id: obj.data.id}
                    }).then((res) => {
                        if (res.code === 0) {
