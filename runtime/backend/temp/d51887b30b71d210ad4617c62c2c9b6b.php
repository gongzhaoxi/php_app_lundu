<?php /*a:2:{s:62:"/www/wwwroot/lundu.ecloudm.com/addons/curd/view/gen/index.html";i:1755524162;s:92:"/www/wwwroot/lundu.ecloudm.com/addons/curd/view/../../../app/backend/view/common/layout.html";i:1764860923;}*/ ?>
<!DOCTYPE html>
        ...
        <div class="layui-card-body">
            ...
            <script type="text/html" id="toolbar">
                <div class="layui-btn-container">
                    <a class="layui-btn layui-btn-sm layui-btn-default <?php echo check_perms('import'); ?>" lay-event="import">
            ...
            <script type="text/html" id="table-operate">
                <button type="button" class="layui-btn layui-btn-xs layui-btn-primary layui-border-green <?php echo check_perms('preview'); ?>" lay-event="preview" lay-tips="预览">
                    <i class="layui-icon icon-eye-fill"></i>
                </button>
                <button type="button" class="layui-btn layui-btn-xs layui-btn-default <?php echo check_perms('update'); ?>" lay-event="update" lay-tips="编辑">
                    <i class="layui-icon icon-edit"></i>
                </button>
                <button type="button" class="layui-btn layui-btn-xs layui-btn-warm <?php echo check_perms('synchrony'); ?>" lay-event="synchrony" lay-tips="同步">
                    <i class="layui-icon icon-seckill-flash"></i>
                </button>
                <button type="button" class="layui-btn layui-btn-xs layui-btn-gloomy <?php echo check_perms('exports'); ?>" lay-event="exports" lay-tips="生成">
                    <i class="layui-icon icon-download-arrow"></i>
                </button>
                <button type="button" class="layui-btn layui-btn-xs layui-btn-danger <?php echo check_perms('destroy'); ?>" lay-event="destroy" lay-tips="销毁">
                    <i class="layui-icon icon-del"></i>
                </button>
            </script>
        </div>
...
<script>
    layui.use(function() {
        ...
        let table = waitUtil.table({
            ...
            ,url: '<?php echo route("addons/curd/gen/index"); ?>'
            ,cols: [[
        ...
        waitUtil.event({
            import: function () {
                waitUtil.popup({
                    title: '选择生成表',
                    url: '<?php echo route("addons/curd/gen/tables"); ?>',
                    area: ['750px', '600px'],
                    success: function (layero, index) {
                        layero.layui.form.on('submit(addForm)', function() {
                            ...
                            waitUtil.ajax({
                                url: '<?php echo route("addons/curd/gen/imports"); ?>',
                                type: 'POST',
            ...
            update: function (obj) {
                waitUtil.popup({
                    title: '更新表结构',
                    url: '<?php echo route("addons/curd/gen/update"); ?>?id='+obj.data.id,
                    area: ['95%', '95%'],
                    success: function (layero, index) {
                        layero.layui.form.on('submit(addForm)', function(data) {
                            ...
                            waitUtil.ajax({
                                url: '<?php echo route("addons/curd/gen/update"); ?>',
                                type: 'POST',
                                data: data.field
                            }).then((res) => {
                                waitUtil.unlock(this);
            ...
            destroy: function (obj) {
                layer.confirm('确定要删除此项数据吗？', function(index) {
                    layer.close(index);
                    waitUtil.ajax({
                        url: '<?php echo route("addons/curd/gen/destroy"); ?>',
                        type: 'POST',
            ...
            preview: function (obj) {
                waitUtil.popup({
                    title: '预览代码',
                    url: '<?php echo route("addons/curd/gen/preview"); ?>?id='+obj.data.id,
                    area: ['95%', '95%'],
                    success: function (layero, index) {}
                });
            },
                    ...
                    if (obj.data.gen_type==='下载') {
                        if (obj.data.menu_type === '自动') {
                            layer.confirm('是否同时创建菜单?', function (i) {
                                layer.close(i);
                                waitUtil.ajax({
                                    url: '<?php echo route("addons/curd/gen/initMenu"); ?>',
                                    type: 'POST',
                                    data: {id: obj.data.id},
                                });
                                window.open('<?php echo route("addons/curd/gen/download"); ?>?id=' + obj.data.id, '_blank');
                            }, function () {
                                window.open('<?php echo route("addons/curd/gen/download"); ?>?id=' + obj.data.id, '_blank');
                            });
                        } else {
                        ...
                        waitUtil.ajax({
                            url: '<?php echo route("addons/curd/gen/exports"); ?>',
                            type: 'POST',
                            data: {id: obj.data.id}
                        });
                    }
            ...
            synchrony: function (obj) {
                layer.confirm(`确定要同步表结构吗？`, function(index) {
                    layer.close(index);
                    waitUtil.ajax({
                        url: '<?php echo route("addons/curd/gen/synchrony"); ?>',
                        type: 'POST',
                        data: {id: obj.data.id}
                    });
                });
            ...
            leave: function () {
                ...
                layer.confirm(`确定要删除选中的${ids.length}项数据吗？`, function(index) {
                    layer.close(index);
                    waitUtil.ajax({
                        url: '<?php echo route("auth.admin/del"); ?>',
                        type: 'POST',
                        data: {ids: ids}
                    }).then((res) => {
                        if (res.code === 0) {
