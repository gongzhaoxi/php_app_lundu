<?php /*a:2:{s:68:"/www/wwwroot/lundu.ecloudm.com/app/backend/view/auth/dept/index.html";i:1755524162;s:66:"/www/wwwroot/lundu.ecloudm.com/app/backend/view/common/layout.html";i:1764860923;}*/ ?>
<!DOCTYPE html>
...
<script>
    layui.use(['treeTable'], function () {
        ...
        let table = layui.treeTable.render({
            ...
            ,url: '<?php echo route("auth.dept/index"); ?>'
            ,toolbar: '#toolbar'
        ...
        waitUtil.event({
            add: function () {
                waitUtil.popup({
                    title: '新增',
                    url: '<?php echo route("auth.dept/add"); ?>',
                    area: ['500px', '450px'],
            ...
            edit: function (obj) {
                waitUtil.popup({
                    title: '编辑',
                    url: '<?php echo route("auth.dept/edit"); ?>?id='+obj.data.id,
                    area: ['500px', '450px'],
                    success: function (layero, index) {
                        layero.layui.form.on('submit(addForm)', function(data) {
                            waitUtil.locking(this);
                            ...
                            waitUtil.ajax({
                                url: '<?php echo route("auth.dept/edit"); ?>',
                                type: 'POST',
                                data: data.field
                            }).then((res) => {
                                waitUtil.unlock(this);
            ...
            del: function (obj) {
                layer.confirm('确定要删除此项数据吗？', function(index) {
                    layer.close(index);
                    waitUtil.ajax({
                        url: '<?php echo route("auth.dept/del"); ?>',
                        type: 'POST',
                        data: {id: obj.data.id}
                    }).then((res) => {
                        if (res.code === 0) {
