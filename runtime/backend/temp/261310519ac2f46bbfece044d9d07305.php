<?php /*a:2:{s:72:"/www/wwwroot/lundu.ecloudm.com/app/backend/view/system/attach/index.html";i:1755524162;s:66:"/www/wwwroot/lundu.ecloudm.com/app/backend/view/common/layout.html";i:1764860923;}*/ ?>
<!DOCTYPE html>
...
<body>
        ...
        <div class="file-container">
            ...
            <div class="file-majors">
                ...
                <div class="folder">
                    <ul class="groups"></ul>
                    <div class="footer">
                        <button type="button" class="layui-btn layui-btn-primary layui-btn-sm cateCreate <?php echo check_perms('attach/cateAdd', false); ?>">添加分组</button>
                    </div>
                </div>
                <!-- 附件 -->
                <div class="attach">
                    ...
                    <div class="header">
                        <div class="layui-btn-group">
                            <button type="button" class="layui-btn layui-btn layui-btn-sm layui-btn-normal <?php echo check_perms('upload/permanent', false); ?>" data-type="10" id="imageUpload">上传图片</button>
                            <button type="button" class="layui-btn layui-btn layui-btn-sm layui-btn-normal layui-hide <?php echo check_perms('upload/permanent', false); ?>" data-type="20" id="videoUpload">上传视频</button>
                            <button type="button" class="layui-btn layui-btn layui-btn-sm layui-btn-normal layui-hide <?php echo check_perms('upload/permanent', false); ?>" data-type="30" id="packageUpload">上传压缩</button>
                            <button type="button" class="layui-btn layui-btn layui-btn-sm layui-btn-normal layui-hide <?php echo check_perms('upload/permanent', false); ?>" data-type="40" id="documentUpload">上传文件</button>
                            <button type="button" class="layui-btn layui-btn layui-btn-sm layui-btn-danger layui-btn-forbid <?php echo check_perms('del', false); ?>" id="delete">删除文件</button>
                            <button type="button" class="layui-btn layui-btn layui-btn-sm layui-btn-warm layui-btn-forbid <?php echo check_perms('move', false); ?>" id="move">
    ...
    <script id="folder" type="text/html">
        <li data-id="-1" class="active">
            <img  src="/static/backend/images/folder.png" alt="icon">
            <span>全部</span>
        </li>
        <li data-id="0">
            <img src="/static/backend/images/folder.png" alt="icon">
    ...
    layui.use(['laytpl', 'laypage', 'upload', 'element'], function () {
        ...
        let event = waitUtil.event({
            ...
            group: function (obj) {
                ...
                waitUtil.ajax({
                    url: '<?php echo route("attach/cateLists"); ?>',
                    type: 'GET',
                    fulShow: false,
                    data: {type: type}
                }).then((res) => {
            ...
            lists: function (obj) {
                ...
                waitUtil.ajax({
                    url: '<?php echo route("attach/index"); ?>',
                    type: 'GET',
                    fulShow: false,
                    data: where
                }).then((res) => {
        ...
        $(document).on('click', '.folder .cateCreate', function () {
            layer.prompt({title: '添加分组', value: ' ', formType: 3}, function(text, index){
                ...
                waitUtil.ajax({
                    url: '<?php echo route("attach/cateAdd"); ?>',
                    type: 'POST',
                    fulShow: false,
        ...
        $(document).on('click', '.folder .cateRename', function (e) {
            ...
            layer.prompt({title: '重命分组', value: span, formType: 3}, function(text, index){
                ...
                waitUtil.ajax({
                    url: '<?php echo route("attach/cateRename"); ?>',
                    type: 'POST',
                    fulShow: false,
        ...
        $(document).on('click', '.folder .cateDelete', function (e) {
            ...
            layer.confirm('确定要删除吗?', function (index){
                layer.close(index);
                waitUtil.ajax({
                    url: '<?php echo route("attach/cateDelete"); ?>',
                    type: 'POST',
                    fulShow: false,
        ...
        $(document).on('click', '.attach li p', function (e) {
            ...
            layer.prompt({title: '重命名', value: name, formType: 3}, function(text, index){
                ...
                waitUtil.ajax({
                    url: '<?php echo route("attach/rename"); ?>',
                    type: 'POST',
                    fulShow: false,
        ...
        $(document).on('click', '.attach .dropdown em', function () {
            ...
            waitUtil.ajax({
                url: '<?php echo route("attach/move"); ?>',
                type: 'POST',
                fulShow: false,
        ...
        upload.render({
            elem: '#imageUpload'
            ,url: '<?php echo route("upload/permanent"); ?>?type=picture'
            ,field: 'file'
            ,multiple: true
            ,acceptMime: 'image/*'
            ,accept: 'images'
            ,exts: '<?php echo implode("|", $imageLimit['ext']); ?>'
            ,size: parseInt('<?php echo htmlentities((string) $imageLimit['size']); ?>') / 1024
            ,data: {
                hide: 0,
        ...
        upload.render({
            elem: '#videoUpload'
            ,url: '<?php echo route("upload/permanent"); ?>?type=video'
            ,field: 'file'
            ,multiple: true
            ,acceptMime: 'video/*'
            ,accept: 'video'
            ,exts: '<?php echo implode("|", $videoLimit['ext']); ?>'
            ,size: parseInt('<?php echo htmlentities((string) $videoLimit['size']); ?>') / 1024
            ,data: {
                hide: 0,
                cid: () => { return cid > 0 ? cid : 0; }
            }
        ...
        upload.render({
            elem: '#packageUpload'
            ,url: '<?php echo route("upload/permanent"); ?>?type=package'
            ,field: 'file'
            ,multiple: true
            ,acceptMime: 'file/*'
            ,accept: 'file'
            ,exts: '<?php echo implode("|", $packageLimit['ext']); ?>'
            ,size: parseInt('<?php echo htmlentities((string) $packageLimit['size']); ?>') / 1024
            ,data: {
                hide: 0,
                cid: () => { return cid > 0 ? cid : 0; }
            }
        ...
        upload.render({
            elem: '#documentUpload'
            ,url: '<?php echo route("upload/permanent"); ?>?type=document'
            ,field: 'file'
            ,multiple: true
            ,acceptMime: 'file/*'
            ,accept: 'file'
            ,exts: '<?php echo implode("|", $documentLimit['ext']); ?>'
            ,size: parseInt('<?php echo htmlentities((string) $documentLimit['size']); ?>') / 1024
            ,data: {
                hide: 0,
                cid: () => { return cid > 0 ? cid : 0; }
            }
