        ...
        return new Promise((resolve, reject) => {
            $.ajax({
                ...
                ,success: function (res) {
                    ...
                    }else if (errShow===true && res.code!==0) {
...
function exportData(param,url,limit){
	var load_index 	= layui.layer.load(2, {shade: [0.3,'#000']});
	var key = '';
	param.export_act=1;
	waitUtil.ajax({
		url: url,
		...
		if(count > limit){
			...
			for(var page=1;page<=page_num;page++){					
				param.page=page;					
				var res = layui.jquery.ajax({
					type: "get",
					url: url+(url.indexOf('?')!=-1?'&':'?')+jsonToUrlParam(param),
					cache: false,
					async: false,
					dataType:'json',
				});								
			...
			param.limit=limit;
			layui.layer.close(load_index);	
			location = url+(url.indexOf('?')!=-1?'&':'?')+jsonToUrlParam(param);
		}
	});
}
